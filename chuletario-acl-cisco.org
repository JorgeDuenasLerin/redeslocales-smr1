...
** PLANTEAMIENTO
   ...
   | / | <>                                            | <>                                          |
   |   | ✗ PING (ICMP)                                 | ✗ PING (ICMP)                               |
   |   | ✓ /DNS (UDP con destino 53)/                  | ✓ /DNS (UDP con origen 53)/                 |
   |   | ✓ /TCP si se ha establecido desde la LAN/     | ✓ /TCP si se ha establecido desde la LAN/   |
   |   | ✓ /TCP si es una conexión entrante de la DMZ/ | ✗ TCP si es una conexión entranta de la DMZ |
   |   | ✗ UDP                                         | ✗ UDP                                       |
   |---+-----------------------------------------------+---------------------------------------------|
   #+REVEAL: split
   ...
   | / | <>                                          | <>                                          |
   |   | ✗ PING (ICMP)                               | ✗ PING (ICMP)                               |
   |   | ✓ /DNS (UDP con origen 53)/                 | ✓ /DNS (UDP con destino 53)/                |
   |   | ✓ /TCP si se ha establecido desde la LAN/   | ✓ /TCP si se ha establecido desde la LAN/   |
   |   | ✗ TCP si es una conexión entranta de la DMZ | ✗ TCP si es una conexión entrante de la DMZ |
   |   | ✗ UDP                                       | ✗ UDP                                       |
   |---+---------------------------------------------+---------------------------------------------|
...
** Solución	:noexport:
   ...
   - Internet y la LAN pueden acceder al servidor web, se permite al administrador.
    ...
    #+END_SRC
     ...
     Regla in en Fa1/0
    #+BEGIN_SRC cisco
    access-list 104 permit tcp 100.0.0.1 eq www any
